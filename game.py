...
class RoboticDogSimulation:
    ...
    def check_collision_with_obstacles(self, x, y, buffer=25):
        ...
        dog_rect = pygame.Rect(x - self.dog.size, y - self.dog.size,
                               self.dog.size * 1, self.dog.size * 1)

        for obstacle in self.obstacles:
            obs_rect = pygame.Rect(obstacle.x - buffer, obstacle.y - buffer,
                                   obstacle.width + buffer * 1,
                                   obstacle.height + buffer * 1)
    ...
    def update(self):
        if not self.paused and not self.battery_dead and not self.ball['collected']:
            ...
            self.dog.find_ball((self.ball['x'], self.ball['y']), obstacle_data,self.check_collision_with_obstacles(self.dog.target_x, self.dog.target_y))
